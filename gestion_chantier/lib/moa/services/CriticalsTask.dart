...
import 'package:gestion_chantier/moa/models/Taskcritical.dart';
...
class CriticalTasksService {
  ...
  /// Récupère les tâches critiques pour un promoteur spécifique (utilisé pour l'agrégation MOA)
  Future<List<Task>> getCriticalTasksByPromoterId(int promoterId) async {
    try {
      final response = await _apiService.dio.get(
        '/tasks/critical',
  ...
  /// Agrège les tâches critiques de tous les promoteurs (profil MOA)
  Future<List<Task>> getCriticalTasksAllPromoters({int pageSize = 100}) async {
    try {
      ...
      final results = await Future.wait(
        promoterIds.map((id) => getCriticalTasksByPromoterId(id)),
      );

      // Aplatit et trie par échéance (croissant)
      final List<Task> merged = [for (final list in results) ...list];
      merged.sort((a, b) => a.endDate.compareTo(b.endDate));
      return merged;
    } on DioException catch (e) {
      _handleDioError(e);
    } catch (e) {
      throw Exception('Erreur lors de l\'agrégation des tâches critiques: $e');
    }
  }
  ...
  /// Version MOA sécurisée: agrège pour tous les promoteurs et gère les erreurs
  Future<List<Task>?> getCriticalTasksSafelyAllPromoters({
    int pageSize = 100,
  }) async {
    try {
      final tasks = await getCriticalTasksAllPromoters(pageSize: pageSize);
      return tasks;
    } catch (e) {
      print('Erreur agrégation tâches critiques MOA: $e');
      return [];
    }
  }
